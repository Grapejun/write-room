...
public class ChallengeScheduler {
    ...
    public void checkChallengeRoutine() {
        ...
        for (int i = 0; i < routineParticipationList.size(); i++) {
            //루틴 체크 작업 내용
            System.out.println("========================================");
            System.out.println("챌린지 루틴"+routineParticipationList.get(i).getId()+" achieveCount: "+routineParticipationList.get(i).getAchieveCount());
            System.out.println("챌린지 루틴"+routineParticipationList.get(i).getId()+" status: "+routineParticipationList.get(i).getChallengeStatus());
            checkRoutineAchieve(routineParticipationList.get(i));
            System.out.println("체크 후");
            System.out.println("챌린지 루틴"+routineParticipationList.get(i).getId()+" achieveCount: "+routineParticipationList.get(i).getAchieveCount());
            System.out.println("챌린지 루틴"+routineParticipationList.get(i).getId()+" status: "+routineParticipationList.get(i).getChallengeStatus());
        }

        for (int i = 0; i < goalsParticipationList.size(); i++) {
            //목표량 체크 작업 내용
            System.out.println();
            System.out.println("챌린지 목표량"+goalsParticipationList.get(i).getId()+" status: "+goalsParticipationList.get(i).getChallengeStatus());
            checkGoalsAchieve(goalsParticipationList.get(i));
            System.out.println("체크 후");
            System.out.println("챌린지 목표량"+goalsParticipationList.get(i).getId()+" status: "+goalsParticipationList.get(i).getChallengeStatus());
            System.out.println("========================================");
        }
    ...
    public void checkGoalsAchieve(ChallengeGoalsParticipation cgp) {
        if (LocalDate.now().minusDays(1).isEqual(cgp.getChallengeGoals().getDeadline())) { //어제가 마감일이라면, 목표량을 채우지 못한 경우만 남음. 채웠으면 노트 저장할 때 이미 SUCCESS로 바뀌니까.
